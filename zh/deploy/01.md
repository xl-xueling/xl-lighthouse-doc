...
* 如果服务器root账号之前配置过无密码登录，请首先将无密码登录配置删除掉（rm -rf /root/.ssh），这个配置可能会与部署脚本有冲突，这个问题我会在后续版本中修复。
...
执行：./bin/tools/examples/start_example.sh，自动创建出演示项目，可用于初步体验XL-LightHouse或测试服务部署是否正常！
...
## 集群性能优化(按集群状况直接拷贝相关配置即可，大部分情况下不需要关注)
