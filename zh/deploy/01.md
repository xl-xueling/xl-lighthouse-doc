...
请使用JDK11编译，编译服务器内存不能小于4G，执行命令如下，该步骤涉及前端模块编译，耗时较长，请耐心等待！
...
系统在部署过程中会出现个别告警信息，不需要处理，不会影响正常运行！
...
部署完成后默认使用内网地址访问，如果您是使用公网IP访问，出现远程连接异常时，可以配置一下安装目录./light-webapps/build/config.json中的REACT_APP_BASE_URL参数的IP地址，将其修改为web服务的公网IP，同时确保您的服务器已对公网放开9089端口(后端接口服务)和8181端口(Web服务)！
...
**按集群节点内存大小拷贝相关配置即可[集群模式参数配置](/management/02.md)或[单机模式参数配置](/management/09.md)，绝大多数情况下不需要自己优化**
